...
from statsScript.utils import is_own_team, is_opp_team, get_side, ID, print3n
from statsScript.player import Player
from statsScript.round import Round
...
class Match:
    def __init__(self, src):
        ...
        self.score = Score(
            own=list(
                filter(is_own_team, src["rounds"][-1]["teams"]))[0]["score"],
            opp=list(
                filter(is_opp_team, src["rounds"][-1]["teams"]))[0]["score"],
        )
        self.own_bans = ""  # Not possible to get as of august 18th 2023
        self.opp_bans = ""  # Not possible to get as of august 18th 2023
        self.rounds = []
        self.starting_side = get_side(src)
        self.players = {
            x: Player(name=x, rounds=len(src["rounds"]))
            for x in self._get_team_players(src)
        }
    ...
    def _show_general(self):
        ...
        data = [
            [
                self.map,
                f"{self.score.own} - {self.score.opp}",
                self.own_bans,
                self.opp_bans,
            ],
        ]
    ...
    def _show_rounds(self):
        ...
        data = [
            [x.number, x.map, x.site, x.side, x.won, x.win_condition]
    ...
    def _kost(self, rnd, player):
        return (
            self._got_kills(rnd["stats"], player)
            or self._planted_or_disabled_defuser(rnd["matchFeedback"], player)
            or self._survived(rnd["stats"], player)
            or self._was_traded(rnd["matchFeedback"], player)
        )
    ...
    def _do_defuser_plants(self, events):
        for event in events:
            if (
                event["type"]["id"] == ID.DEFUSER_PLANTED_ID.value
                and event["username"] in self.players
            ):
                player = self.players[event["username"]]
                player.plants += 1
    ...
    def _do_defuser_disables(self, events):
        for event in events:
            if (
                event["type"]["id"] == ID.DEFUSER_DISABLED_ID.value
                and event["username"] in self.players
            ):
                player = self.players[event["username"]]
                player.disables += 1
    ...
    def _do_clutches(self, round_stats):
        players_alive = [
            ...
            if self._survived(round_stats, player["username"])
        ]
    ...
    def _planted_or_disabled_defuser(self, events, player):
        for event in events:
            if (
                event["type"]["id"] == ID.DEFUSER_DISABLED_ID.value
                or event["type"]["id"] == ID.DEFUSER_PLANTED_ID.value
            ) and event["username"] == player:
                return True
        return False
    ...
    def _was_traded(self, events, player):
        ...
        for event in events:
            ...
            if not (
                event["type"]["id"] == ID.KILL_ID.value
                and event["username"] in self.players
            ):
